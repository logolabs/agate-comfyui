...
app.registerExtension({
  ...
  beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    // Ensure widgets stop above the brand strip.  LiteGraph's _arrangeWidgets
    // uses bodyHeight (a getter that returns renderingSize[1]) to decide how
    // much vertical space widgets can occupy.  Without this override, multiline
    // text widgets (prompt, negative_prompt) expand into the brand strip.
    let cur = nodeType.prototype;
    let desc = null;
    while (cur && !desc) {
      desc = Object.getOwnPropertyDescriptor(cur, "bodyHeight");
      if (!desc) cur = Object.getPrototypeOf(cur);
    }
    if (desc && desc.get) {
      Object.defineProperty(nodeType.prototype, "bodyHeight", {
        get() {
          const val = desc.get.call(this);
          return Math.max(0, val - BRAND.strip);
        },
        configurable: true,
      });
    }

    // Chain with any existing onDrawForeground (e.g. from other extensions).
